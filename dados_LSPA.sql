...
FROM produtos p
JOIN safra s ON p.cod_produto = s.cod_produto
...
SELECT p.nome_produto, s.ano, sum(s.volume) as volume_total FROM produtos p
...
SELECT p.nome_produto, avg(s.volume) as volume_medio  FROM produtos p
INNER JOIN safra s  ON s.cod_produto = p.cod_produto group by p.nome_produto
having avg(s.volume) > 100000000;

-- Listar os produtos e suas safras ordenados pelo ano e mês da safra 
SELECT p.nome_produto, s.mes, s.ano  FROM produtos p
INNER JOIN safra s  ON s.cod_produto = p.cod_produto order by s.ano , s.mes ;
...
-- crie uma análise no Power BI desses dados. Para isso, execute o seguinte comando:
